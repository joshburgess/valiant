module Hsqlx.CLI.CustomTypesSpec (spec) where

import Data.Aeson (encode, object, (.=))
import Data.ByteString.Lazy qualified as LBS
import Data.Map.Strict qualified as Map
import Database.PostgreSQL.LibPQ (Oid (..))
import Hsqlx.CLI.CustomTypes (loadCustomTypes)
...
spec = do
  describe "loadCustomTypes" $ do
    it "returns empty map when file does not exist" $ do
      customs <- loadCustomTypes "/nonexistent/hsqlx-types.json"
      customs `shouldBe` Map.empty

    it "loads custom type mappings from JSON file" $ do
      withSystemTempDirectory "hsqlx-test" $ \dir -> do
        let path = dir <> "/hsqlx-types.json"
            content =
              encode
                [ object
                    [ "oid" .= (600 :: Int)
