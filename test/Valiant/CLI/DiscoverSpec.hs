module Valiant.CLI.DiscoverSpec (spec) where

import Data.ByteString.Char8 qualified as BS8
import Data.List (sort)
import Valiant.CLI.Discover
...
spec = do
  describe "discoverSqlFiles" $ do
    it "finds .sql files recursively" $ do
      withSystemTempDirectory "valiant-test" $ \tmpDir -> do
    ...
    it "returns empty for non-existent directory" $ do
      files <- discoverSqlFiles "/tmp/valiant-nonexistent-dir-12345"
