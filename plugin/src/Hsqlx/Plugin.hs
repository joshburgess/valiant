...
import Hsqlx.Plugin.Rewrite (rewriteModule)
...
import Hsqlx.Plugin.Verify (verifyQueryFile)
...
-- | The hsqlx GHC source plugin.
plugin :: Plugin
plugin =
  defaultPlugin
    { parsedResultAction = hsqlxRewrite
    , typeCheckResultAction = hsqlxTypeCheck
    , pluginRecompile = hsqlxRecompile
    }

-- | Before typechecking, rewrite queryFile calls to mkStatement calls.
hsqlxRewrite :: [CommandLineOption] -> ModSummary -> ParsedResult -> Hsc ParsedResult
hsqlxRewrite opts _modSummary parsedResult = do
