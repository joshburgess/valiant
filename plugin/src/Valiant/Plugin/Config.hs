-- | Internal: plugin option parsing for the @valiant-plugin@. The
-- public entry point is "Valiant.Plugin"; this module is exposed
-- only so the plugin's test suite can exercise it. Not stable API.
...
parseOptions = List.foldl' applyOpt defaultConfig
  where
