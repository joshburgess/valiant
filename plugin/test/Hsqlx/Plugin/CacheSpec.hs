module Hsqlx.Plugin.CacheSpec (spec) where

import Control.Exception (SomeException, try)
import Hsqlx.Plugin.Cache
...
spec = do
  ...
  describe "findCacheFile" $ do
    it "returns Nothing for non-existent cache directory" $ do
      result <- findCacheFile "/tmp/hsqlx-nonexistent-cache-dir" "test.sql" "abc123"
      result `shouldBe` Nothing

    it "returns Nothing when no matching file exists" $ do
      withSystemTempDirectory "hsqlx-cache-test" $ \tmpDir -> do
        result <- findCacheFile tmpDir "test.sql" "abc123"
        result `shouldBe` Nothing

  describe "readCacheEntry" $ do
    it "reads an actual .hsqlx cache file written by the CLI" $ do
      -- Tests compatibility between CLI-written cache and plugin reader
      let cacheFile = "/Users/joshburgess/code/hsqlx/.hsqlx/posts-find_by_id-b63f2b90e455.json"
      exists <- doesFileExist cacheFile
      if not exists
        then pendingWith "No .hsqlx cache files available (run hsqlx prepare first)"
        else do
