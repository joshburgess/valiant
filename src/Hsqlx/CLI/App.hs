module Hsqlx.CLI.App
  ( run
  ) where

import Hsqlx.CLI.Command.Check (runCheck)
import Hsqlx.CLI.Command.Generate (GenerateOpts (..), runGenerate)
import Hsqlx.CLI.Command.Prepare (runPrepare)
import Hsqlx.CLI.Command.Types (runTypes)
import Hsqlx.CLI.Command.Watch (runWatch)
import Hsqlx.CLI.Config (AppEnv (..), resolveEnv)
import Options.Applicative
import System.Exit (ExitCode, exitWith)

-- | Entry point for the @hsqlx@ CLI.
run :: IO ()
run = do
...
parserInfo =
  info
    (optsParser <**> helper)
    ( fullDesc
        <> header "hsqlx — compile-time checked SQL for Haskell"
