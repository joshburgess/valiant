...
| 20 | **LargeObject silent parse failures** | Replace `pure 0`/`pure empty` with `throwPgWire (DecodeError ...)` | `0fdd7b0`, `62cd234` |
...
| 27 | **LargeObject `fail` throws `IOException` not `PgWireError`**: breaks library error contract | Replace all `fail` with `throwPgWire (DecodeError ...)` | `62cd234` |
...
| 30 | **`Dynamic.hs` `fail` throws `IOException`**: inconsistent with `PgWireError` | Replace with `throwPgWire (DecodeError ...)` | `aeb7ebb` |
