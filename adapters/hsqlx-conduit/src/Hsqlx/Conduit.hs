...
-- | Conduit streaming adapter for hsqlx.
--
  ...
  where
    go !acc = do
      ...
      case msg of
        ...
        other -> throwHsqlx (ProtocolError ("Unexpected in fold source: " <> BS8.pack (show other)))
...
freshCursorName = do
  ...
  pure ("hsqlx_conduit_" <> BS8.pack (show n))
