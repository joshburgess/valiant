...
withSchema conn action = do
  _ <- simpleQuery conn "DROP TABLE IF EXISTS users_effectful CASCADE"
  _ <- simpleQuery conn
    "CREATE TABLE users_effectful (\
...
insertTestUsers conn = do
  _ <- simpleQuery conn
    "INSERT INTO users_effectful (name, email) VALUES \
...
stmtListAll = mkStatement
  "SELECT id, name FROM users_effectful ORDER BY id"
  [] ["id", "name"] "<test>"

stmtSelectOne :: Statement Int32 (Int32, Text, Maybe Text)
stmtSelectOne = mkStatement
  "SELECT id, name, email FROM users_effectful WHERE id = $1"
