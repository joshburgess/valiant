...
-- | Streamly streaming adapter for hsqlx.
--
  ...
  where
    go !acc = do
      ...
      case msg of
        ...
        other -> throwHsqlx (ProtocolError ("Unexpected in fold stream: " <> BS8.pack (show other)))
  ...
  where
    go = do
      ...
      case msg of
        ...
        ErrorResponse err -> throwHsqlx (QueryError err)
        NoticeResponse _ -> go
        other -> throwHsqlx (ProtocolError ("Unexpected in DECLARE: " <> BS8.pack (show other)))
...
freshCursorName = do
  ...
  pure ("hsqlx_streamly_" <> BS8.pack (show n))
