...
import Hsqlx.Bluefin (runHsqlxB, fetchAllB, fetchOneB, executeB)
  ...
  describe "Hsqlx.Bluefin" $ do
    ...
    it "fetchOneB returns Just for existing row" $ do
      withTestConnection $ \conn -> withSchema conn $ do
        insertTestUsers conn
        pool <- mkPool
        result <- runHsqlxB pool $ \db -> fetchOneB db stmtSelectOne 1
        closePool pool
        case result of
    ...
    it "executeB returns rows affected" $ do
      withTestConnection $ \conn -> withSchema conn $ do
        ...
        n <- runHsqlxB pool $ \db -> executeB db stmtDel (1 :: Int32)
        closePool pool
        n `shouldBe` 1
    ...
    it "composes multiple operations" $ do
      withTestConnection $ \conn -> withSchema conn $ do
        insertTestUsers conn
        pool <- mkPool
        (users, mUser) <- runHsqlxB pool $ \db -> do
