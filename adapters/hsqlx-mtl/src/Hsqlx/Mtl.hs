...
import Hsqlx (Connection, IsolationLevel, Pool, PoolStats, Statement, Transaction)
...
forEachMtl stmt params action = do
  pool <- getPool
  liftIO $ withResource pool $ \conn -> Hsqlx.forEach conn stmt params action
