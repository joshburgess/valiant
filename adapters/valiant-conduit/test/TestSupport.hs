...
withSchema conn action = do
  _ <- simpleQuery conn "DROP TABLE IF EXISTS users_conduit CASCADE"
  _ <- simpleQuery conn
    "CREATE TABLE users_conduit (\
...
insertTestUsers conn = do
  _ <- simpleQuery conn
    "INSERT INTO users_conduit (name, email) VALUES \
...
stmtListAll = mkStatement
  "SELECT id, name FROM users_conduit ORDER BY id"
  [] ["id", "name"] "<test>"

stmtSelectOne :: Statement Int32 (Int32, Text, Maybe Text)
stmtSelectOne = mkStatement
  "SELECT id, name, email FROM users_conduit WHERE id = $1"
